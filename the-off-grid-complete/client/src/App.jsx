...
export default function App() {
  ...
  const searchSuggestions = useMemo(() => {
    ...
    return products.map((p) => {
      ...
      if (targetCategory && category === targetCategory) score += 6;
      if (category.startsWith(q)) score += 3;
      if (tokens.length > 1 && tokens.every((token) => haystack.includes(token))) score += 3;
      return score ? { product: p, score } : null;
    }).filter(Boolean).sort((a, b) => b.score - a.score || Number(b.product.id || 0) - Number(a.product.id || 0)).slice(0, 5).map((item) => item.product);
  }, [products, searchText]);
