...
export default function TrackOrder({ orders = [] }) {
  ...
  const [order, setOrder] = useState(existing || null); const [loading, setLoading] = useState(!existing); const [error, setError] = useState(""); const [refreshing, setRefreshing] = useState(false); const [history, setHistory] = useState([]);

  const loadOrder = async (silent = false) => { if (!id) return; if (!silent) setLoading(true); else setRefreshing(true); try { const data = await api(`/orders/${id}`); setOrder(data); setError(""); } catch (err) { setError(err.message || "Order not found"); } finally { setLoading(false); setRefreshing(false); } };
  const loadHistory = async () => { if (!id) return; try { const data = await api(`/orders/${id}/history`); setHistory(Array.isArray(data) ? data : []); } catch { setHistory([]); } };

  useEffect(() => { if (existing) { setOrder(existing); setLoading(false); } else loadOrder(); loadHistory(); }, [id, existing]);
  useEffect(() => { if (!id || order?.status === "delivered" || order?.status === "cancelled") return; const timer = setInterval(() => { loadOrder(true); loadHistory(); }, 30000); return () => clearInterval(timer); }, [id, order?.status]);

  const current = order ? steps.indexOf(order.status) : -1; const displayId = order ? `OG${String(order.id).padStart(6, "0")}` : id; const eta = useMemo(() => etaFor(order), [order]); const items = Array.isArray(order?.items) ? order.items : []; const itemCount = items.reduce((total, item) => total + Number(item.quantity || 0), 0);
  ...
  return <div className="page track-page">
    ...
    {loading ? <div className="empty-box"><h2>LOADING ORDER...</h2></div> : order ? <div className="track-card">
      ...
      {order.status === "cancelled" ? <p className="notify-me-error">This order was cancelled and its reserved stock was released.</p> : <>
        ...
        {history.length > 0 && <div className="track-history"><div className="track-items"><h3>STATUS UPDATES</h3>{[...history].reverse().map((event) => <div className="track-history-row" key={event.id}><b><CheckCircle2 size={14} /></b><div><strong>{labels[event.status] || String(event.status).toUpperCase()}</strong><span>{event.note || "Order status updated."}</span></div><time>{formatDate(event.created_at)}</time></div>)}</div></div>}
      ...
      <button className="text-button" type="button" onClick={() => navigate("/orders")}>BACK TO ORDERS</button><button className="text-button" type="button" onClick={() => { loadOrder(true); loadHistory(); }} disabled={refreshing} style={{ marginLeft: 18 }}><RefreshCw size={13} style={{ verticalAlign: "middle", marginRight: 5 }} />{refreshing ? "REFRESHING" : "REFRESH STATUS"}</button>
